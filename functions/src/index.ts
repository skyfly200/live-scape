...
exports.sendWelcomeEmail = functions.auth.user().onCreate((user) => {
  ...
  db.collection('user').doc(user.uid).set(profile)

  return true //sendWelcomeEmail(email, displayName)
})
...
exports.sendByeEmail = functions.auth.user().onDelete((user) => {
  ...
  db.collection('user').doc(user.uid).update({ active: false })

  return true //sendGoodbyeEmail(email, displayName)
})
// [END sendByeEmail]
